...
<body>
	...
	<script type="text/javascript">
		//表格
		$('#dg').datagrid({    
		    url:'/sms/student/loadStudents.do',
		    pagination:true,
		    rownumbers:true,
		    striped:true,
		    pageSize:10, 
		    ...
		    toolbar: [{
				...
				handler: function(){
					...
					$.messager.confirm('警告','您确认想要删除选中的【'+rows.length+'】个学生吗？删除学生后该学生的状态会变为已休学！',function(r){    
					    if (r){    
							$.post('/sms/student/deleteStudent.do',{midStr:midStr},function(data){
								if(data=='true'){
									$('#addwin').window('close');
									$('#dg').datagrid('reload',{});
		...
		function addStudent() {
			...
			$.post('/sms/student/addStudent.do',{name:name,zip:zip,indate:indate,deptId:deptId,professId:professId},function(data){
				if(data=='true'){
		...
		function editStudent() {
			...
			$.post('/sms/student/editStudent.do',{sno:sno,name:name,zip:zip,indate:indate,deptId:deptId,professId:professId,state:state},function(data){
				if(data=='true'){
		...
		function getProfessByDeptId(flag) {
			...
			//修改
			if(flag==2){
				$.post("/sms/profess/queryAllProfessByDeptId.do",{deptId:$('#deptEdit').val()},function(data){
					$('#professEdit option:not(:first)').remove();
					for (var i = 0; i < data.length; i++) {
