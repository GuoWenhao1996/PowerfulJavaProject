...
<body>
	...
	<script type="text/javascript">
		//表格
		$('#dg').datagrid({    
		    url:'<%=basePath%>/menu/loadMenus.do',
		    pagination:true,
		    rownumbers:true,
		    striped:true,
		    pageSize:10, 
		    ...
		    toolbar: [{
				...
				handler: function(){
					...
					$.messager.confirm('警告','您确认想要删除选中的【'+rows.length+'】条菜单吗？删除菜单的同时对应的权限也会被删除！',function(r){    
					    if (r){    
							$.post('<%=basePath%>/menu/deleteMenu.do',{midStr:midStr},function(data){
								if(data=='true'){
									$('#addwin').window('close');
									$('#dg').datagrid('reload',{});
				...
				handler: function(){
					...
					var staObj = $(".grade").find("[switchbuttonName='grade']");
					if(rows[0].parentid==0){
						staObj.switchbutton("check");
					}else{
						staObj.switchbutton("uncheck");
						$.post("<%=basePath%>/menu/getParentMenu.do",function(data){
							$('#parentMenuEdit option:not(:first)').remove();
							for (var i = 0; i < data.length; i++) {
								if(data[i].mid==rows[0].parentid){
									$('#parentMenuEdit').append('<option selected value="'+data[i].mid+'">'+data[i].name+'</option>');
								}else{
		...
		function addMenu() {
			...
			$.post('<%=basePath%>/menu/addMenu.do',{name:name,url:url,pid:pid},function(data){
				if(data=='true'){
		...
		function editMenu() {
			...
			$.post('<%=basePath%>/menu/editMenu.do',{mid:mid,name:name,url:url,pid:pid},function(data){
				if(data=='true'){
